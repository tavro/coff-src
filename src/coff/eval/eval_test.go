...
func TestHashIdxExpressions(t *testing.T) {
	tests := []struct {
		input string
		expected interface{}
	} {
...
func TestHashLiterals(t *testing.T) {
	...
	expected := map[object.HashKey]int64{
		(&object.Str{Value: "one"}).HashKey(): 1,
		(&object.Str{Value: "two"}).HashKey(): 2,
		(&object.Str{Value: "three"}).HashKey(): 3,
	...
	for expectedKey, expectedValue := range expected {
		...
		testIntObject(t, pair.Value, expectedValue)
	}
}
...
func TestStdFunctions(t *testing.T) {
	tests := []struct {
		...
		{`len("four")`, 4},
		{`len("hello world")`, 11},
		{`len(1)`, "argument to `len` is not supported, got INT"},
	...
	for _, tt := range tests {
		...
		case int:
			testIntObject(t, evaluated, int64(expected))
		case string:
			errObj, ok := evaluated.(*object.Error)
			if !ok {
...
func TestArrLiterals(t *testing.T) {
	input := "[1, 2 * 2, 3 + 3]"
	evaluated := testEval(input)

	result, ok := evaluated.(*object.Arr)
	if !ok {
	...
	testIntObject(t, result.Elements[0], 1)
	testIntObject(t, result.Elements[1], 4)
	testIntObject(t, result.Elements[2], 6)
}

func TestArrIdxExpressions(t *testing.T) {
	tests := []struct {
		input string
		expected interface{}
	} {
